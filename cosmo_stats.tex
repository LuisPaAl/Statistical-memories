...
La descripci\'on est\'andar de las propiedades din\'amicas del Universo se basa en la tero\'ia 
de la Relatividad General de Einstein, la cual construye una conecci\'on entre la geometr\'ia del 
...
$g_{\mu \nu}$. \'Estas en conjunto dirigen el comportamiento de los constituyentes de universo  $T_{\mu \nu}$.
Una modificaci\'on aceptable a las ecuaciones de Einstein es la introducci\'on de una  invariante
bajo transformaciones de Loretz $\Lambda g_{\mu \nu}$ en las ecuaciones de campo
%
...
Las ecuaciones (\ref{eq:Einstein}) son, en general, un conjunto de ecuaciones diferenciales parciales acopladas 
cuasi-lineares y de segundo orden para los diez elementos del tensor m\'etrico. Sin embargo, \'estas pueden 
presentar soluciones anal\'icas simples en la presencia de simetr\'ias gen\'ericas, como veremos mas adelante.
...
cuando se promedia sobre escalas suficientemente grandes, a un tiempo particular, el Universo observable 
puede considerarse como \textit{Homog\'eneo e Isotr\'opico} con gran presici\'on.
Por ejemplo, en escalas superiores a $100$ Mega-parsecs\footnote{1 parsec (pc) $\simeq 3.261$ a\~nos luz} 
...
observada en la distribuci\'on de temperaturas vista a trav\'es de la radiaci\'on C\'osmica en el Fondo de
Microondas (CMB), una parte en $10^5$ es la mejor evidencia observacional que tenemos en favor de un 
universo isotr\'opico. Por tanto, s\'i la isotrop\'ia se da por sentado y teniendo en cuenta que nuestra
posici\'on ocupada en el Universo no tiene preferencia sobre ninguna otra -conocido como el \textit{Principio 
Copernicano}- por tanto homogeneidad se sigue de considerar isotrop\'ia en cada punto.
 \\
 
 	\fbox{
	 	\parbox[c][2.cm][c]{0.95\textwidth\linespread{1.5}\selectfont{}}
			{
...
El primero demanda que todos lo puntos sobre una hiper-superficie espacial son equivalentes,
mientras que el segundo asegura que todas las direcciones sobre dicha hiper-superficie son 
...
Por lo tanto, se sigue que la m\'etrica deba tener la siguiente forma.
%
	\beq \label{eq:FRW}
		ds^2 = g_{\mu \nu} dx^\mu dx^\nu = dt^2 - a^2(t)\gamma_{ij}dx^i dx^j,
	\eeq
%
donde $a(t)$ es una funci\'on - comúnmente denominada como {\it factor de escala normalizado} - 
cuya dependencia solo es temporal.
A trav\'es de los a\~nos, observaciones cosmológicas han dado evidencia contundente que 
el Universo actual se encuentra en un per\'iodo de expansi\'on y por tanto el factor de escala satisface 
la condici\'on $\dot a(t)>0$.
%
La m\'etrica que describe este Universo homogeneo e isotr\'opico sobre las escalas m\'as grandes se le conoce
como m\'etrica de Friedmann-Robertson-Walker (FRW).
...
Por tanto, sustituyendo (\ref{eq:FRW2}) en (\ref{eq:FRW}), la m\'etrica FRW se reescribe de la siguiente forma
	...
	\eeq
%
La constante $K$ describe la geometr\'ia de las secciones espaciales y dependiendo del valor $K>0, K=0$ o $K<0$
...
%
	\begin{eqnarray}
		R^0_0 &=& -\frac{3\ddot a}{a},\label{eq:curvs1} \label{eq:R1}\\
		R^i_j &=& \left(\frac{\ddot a}{a}+\frac{2\dot a^2}{a^2} +
				  \frac{2\kappa}{a^2} \right)\delta^i_j, \label{eq:R2}\\
		R     &=& 6 \left(\frac{\ddot a}{a} +\frac{\dot a^2}{a^2}+ 
				  \frac{\kappa}{a^2} \right). \label{eq:R3}
	\end{eqnarray}


Por tanto, la din\'amica del espacio-tiempo en un universo homogeneo e isotr\'opico se reduce a determinar
...
La geomtetr\'ia del espacio-tiempo queda determinada por las ecuaciones (\ref{eq:R1})-(\ref{eq:R3}) y por tanto 
para resolver las ecuaciones de Einstein solo falta especificar el contenido de materia. 
...
%
A partir de \'este, se puede obtener la conservaci\'on de la energ\'ia $\nabla_\mu T^\mu_\nu=0$,
la cual gu\'ia a la ecuaci\'on de continuidad
%
	\beq
...
\noindent
Para poder entender las propiedades din\'amicas del Universo, se necesita primero tener en cuenta
el contenido total de \'este. Nuestro enfoque se basa en los fluidos perfectos barotr\'opicos que satisfacen 
una {\it ecuaci\'on de estado} dependiente del tiempo $w(a)$ de la forma
%
	\beq
		p = w(a)\rho,
	\eeq
...
\noindent
para cualquier componente. Si adem\'as se considera $w$ igual a una constante, la ecuaci\'on de continuidad 
...
ecuaci\'on de Friedmann nos lleva a la evoluci\'on temporal del factor de escala
%
	\beq
...
oscura fria (DM) y la energ\'ia del vacio $\Lambda$. El comportamiento de cada uno de estos
...
\noindent
La densidad de energ\'ia de la radiaci\'on $\rho_r$ total en el Universo puede describirse en t\'erminos
...
La densidad de energ\'ia  de los neutrinos sin masa en el fondo c\'osmico (estimada a partir de argumentos te\'oricos) esta
dada por 
%
	\beq \label{eq:nu}
...
\noindent
donde $N_{\rm eff}$ es el n\'umero efectivo de especies de neutrinos; n\'otese que considerando el modelo est\'andar $N_{\rm eff}=3.046$.
Recientemente varios experimentos sugieren que los neutrinos efectivamente si poseen 
una masa aunque \'esta muy peque\~na, por ejemplo experimentos que detectan neutrinos atmosf\'ericos o neutrinos solares.
...
La densidad de materia total puede expresarse como la suma de las contribuciones barionicas (b)
...
\textit{Bariones}. Son las bases de la materia que f\'acilmente localizamos alrededor de nuestro universo 
...
un n\'umero igual de protones que de electrones ($e^-$ - leptones cargados). 
Un art\'iculo de revisi\'on sobre la formaci\'on de elementos en el bing bang puede encontrarse en [].
\\



\textit{Materia oscura}. La existencia de materia oscura no-bari\'onica puede ser inferida a 
...
y motivaciones te\'oricas es presentado por [].
...
\'este puede ser asociado a la densidad de energ\'ia del vacio, dado por 
%
	\beq
...
\noindent
Extrapolando nuestras teor\'ias hacia el futuro. donde la materia y radiac\'on son diluidas debido a la expansi\'on del Universo, 
...
La constante cosmol\'ogica se puede ver como la forma m\'as simple de una componente de \textit{energ\'ia oscura}, 
com\'unmente considerada como el principal candidato para dar explicaci\'on de la aceleraci\'on actual del cosmos.
En [] encuentran que una ecuaci\'on de estado dependiente del tiempo $w_{DE}(a)$ provee una mejor descripci\'on de las
...
\noindent
Por tanto esta densidad de energ\'ia queda descrita por una ecuaci\'on de estado $w_k=-1/3$ para la cual
el factor de escala evoluciona proporcionalmente al tiempo c\'osmico $a\propto t$. El
comportamiento general del t\'ermino de curvatura se entiende f\'acilmente si la ecuaci\'on de Friedmann
se reescribe (sin considerar la constante cosmol\'ogica) de la siguiente manera
%
	\beq \label{eq:curv}
...
posee una curvaruta plana ($k=0$), a este valor de la densidad se le conoce como la \textit{densidad cr\'itica}, y esta dada por
%
	\beq
...
queda englobada en el factor de escalar.
...
Los valores de estas funciones al d\'ia de hoy usualmente son denotados como $H_0\equiv H(t_0)$
y $q_0 \equiv q(t_0)$.
...
En esta secci\'on se da una breve introducci\'on a las cantidad utilizadas en la descripci\'on de las propiedades
del cosmos.
...
Estos, sin embargo, no son predichos por una teori\'ia fundamental sino que tienen que ser modulados
a mano de tal manera que la mejor combinaci\'on provee la mejor descripci\'on de las observaciones
...
las distancias luminosas medidas y por tanto guiarnos a diferentes realizaciones de nuestro 
...
mediciones del sat\'elite WMAP encuentran que $\Omega_{\gamma,0} = 2.469 \times 10^{-5} h^{-2}$  correspondiente 
a una temperatura $T_{cmb}=2.725K$. 
De manera muy similar para los neutrinos $\nu$ mientras estos mantengan un comportamiento relativista, ya que pueden 
relacionanarse con la densidad de los fotones a trav\'es de (\ref{eq:nu}). 
Sin embargo, variaciones del resto de los par\'ametros deja distintos tipos de huellas en la historia de fondo y la evoluci\'on
de las perturbaciones observadas a trav\'es de las distancias luminosas.
La figura () muestra los valores t\'eoricos del modulo de las distancias para tres modelos diferentes tomando en cuenta
combinaciones de $\Omega_{m,0}$ y $\Omega_{\Lambda,0}$. N\'otese que los objetos parecen estar m\'as alejados 
(se observan m\'as tenues) en un universo con constante cosmol\'ogica que en uno dominado solo por materia.
...
cosmol\'ogicos comparandola con los datos observacionales. Sin embargo, esta parametrizaci\'on no es unica y alguna otra 
pueden ser tan buena como \'esta. Algunas parametrizaciones hacen uso del conocimiento de la f\'isica o de la 
...
distancias luminosas a partir de explosiones de Supernovas tipo IA y formaci\'on de estructura
a gran escala. En este art\'iculo nos enfocaremos en las supernovas.
...
Por m\'as de casi dos d\'ecadas las observaciones en las supernovas han proporcionado 
evidencia decisiva que la expansi\'on actual del universo se encuentra en una etapa de 
aceleramiento. En particular estudios de supernovas tipo IA consideradas como velas est\'andar:
ellas tiene la propiedad de poseer la misma luminosidad intr\'inseca con alta presici\'on, hasta un factor 
de reescalamiento. Por tanto, la aceleraci\'on actual sugiere de la existencia de una componente ex\'otica
o la posibilidad de teor\'ias alternativas las cuales producir\'ian tal efecto. Algunos ejemplos 
de detecci\'on de supernovas tipo IA importante mencionar, incluyen:

	\begin{itemize}
		\item El Sloan Digital Sky Survey-II (SDSS) quien descrubri\'o y midi\'o curvas de luz de m\'as de 327 supernovas
		...
		\item Recientemente la compilaci\'on de datos de todas las fuentes antes mencionadas es nombrada
			como Union 2.1
	\end{itemize}

Las mediciones de supernovas pueden ser graficadas en el diagrama de Hubble con el modulo o luminosida de la distancia 
...
\begin{figure}[th]
\begin{center}
\includegraphics[trim =  0mm  0mm 0mm 0mm, clip, width=10cm, height=5cm]{SNs.pdf} 
\end{center}
...
Este art\'iculo nos enfocado en traducir la informaci\'on experimental/observacional en constricciones de nuestros modelos 
...
ocurra dado que tal que $x_1$ ha ocurrido tambi\'en.
\\
...
comparaci\'on entre modelos. Para prop\'ositos de este art\'iculo, omitiremos esta cantidad $P(D)$.
...
De esta manera, el algoritmo completo puede ser descrito por los siguientes pasos:
%
	\begin{enumerate}
...
Es claro la necesidad de alg\'un criterio que nos permita saber cuando nuestras cadenas 
...
es el llamado \textit{criterio de convergencia de Gelman-Rubin}  (1992) [ref]. En \'este se considera que una 
...
Con esto, nuestros datos lucir\'an como se pueden ver en la figura \ref{data}.


\begin{figure}[ht] 
\includegraphics[trim = 1mm  1mm 1mm 1mm, clip, width=8.cm, height=5cm]{data.png}
\caption{\jav{Graficar barras de error}. Datos a lo largo 
...
$a$ y $m$ de nuestra recta $y=a+mx$, por lo que nos gustar\'ia estimar dichos par\'ametros 
mediante un proceso de inferencia Bayesiana. Tambi\'en es posible estimar cual es la desviaci\'on 
estandar de nuestro conjunto de datos a lo largo de la recta, lo que nos agregar\'ia un par\'ametro 
$\sigma$ extra por estimar. Si consideramos que en principio no conocemos nada sobre el valor de 
...
Considerando que existen valores de $a$, $m$ y $\sigma$ para los cuales los datos se fijan mejor 
...
m\'odulo PyMC3 [ref] ya implementado en Python que nos permite emplear este m\'etodo de forma m\'as sencilla. 
Para el lector interesado, el c\'odigo puede encontrarse en [ref]. En nuestro an\'alisis corrimos un total de 
6 cadenas con 10,000 pasos cada una. Nuestro resultado obtenido puede verse en la tabla \ref{tabla1} y la 
figura \ref{chain}. Como podemos ver en el lado izquierdo de la figura, existen regiones para las cuales 
...
de una desviaci\'on est\'andar del valor promedio estimado para estos par\'ametros, mientras que el valor 
real de $\sigma$ parece encontrarse ligeramente por arriba. Adicionalmente se obtuvo el valor del criterio 
...
\begin{figure}[ht] 
\includegraphics[trim = 1mm  1mm 1mm 1mm, clip, width=15.cm, height=8cm]{chain.png}
...
\begin{figure}[ht] 
\includegraphics[trim = 1mm  1mm 1mm 1mm, clip, width=5.cm, height=5cm]{cab.png}
\includegraphics[trim = 1mm  1mm 1mm 1mm, clip, width=5.cm, height=5cm]{cas.png}
\includegraphics[trim = 1mm  1mm 1mm 1mm, clip, width=5.cm, height=5cm]{cas.png}
\caption{Regiones de confidencia en 2D para nuestros par\'ametros $a$, $b$ y $\sigma$. Graficamos los contornos a 1-4 $\sigma$.}% \citep{LiddleLyth}.}
\label{fig:chain}
\end{figure}
